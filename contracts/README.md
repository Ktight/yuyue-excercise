...
`src/**` is the modular source of truth. `openapi.yaml` is generated and must never be edited by hand. The current frozen version is `1.7.0`; module readiness is recorded in `status.json`.
...
See `PHASE_9_DELIVERY.md` for the latest training-plan and class-record linking contract decisions and verification evidence.
