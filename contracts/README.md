...
`src/**` is the modular source of truth. `openapi.yaml` is generated and must never be edited by hand. The current frozen version is `1.6.0`; module readiness is recorded in `status.json`.
