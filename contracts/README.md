...
`src/**` is the modular source of truth. `openapi.yaml` is generated and must never be edited by hand. The current frozen version is `1.9.0`; module readiness is recorded in `status.json`.
...
See `PHASE_11_DELIVERY.md` for the latest management-dashboard and reminder contract decisions and verification evidence.
