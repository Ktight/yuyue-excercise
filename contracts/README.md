...
`src/**` is the modular source of truth. `openapi.yaml` is generated and must never be edited by hand. The current frozen version is `1.8.0`; module readiness is recorded in `status.json`.
...
See `PHASE_10_DELIVERY.md` for the latest student-feedback and live report-preview contract decisions and verification evidence.
