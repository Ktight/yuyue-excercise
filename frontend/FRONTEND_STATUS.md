...
## 五角色导航与移动端质量加固（2026-07-24）

- 新增五角色全页面导航审计，覆盖 59 个桌面主导航入口、22 个手机主导航入口和
  14 个越权路径。
- 每个入口自动检查角色菜单、403/404、控制台错误、页面异常、横向溢出及可见操作名称。
- 修复训练师手机端学员列表筛选栏把 390px 视口撑到 623px 的横向溢出。
- 修复运行时配置单测受本机 `.env.local` 中 Mock 开关影响的问题；默认值测试改为验证
  纯配置函数。
- 完整 `check:all` 通过：452 个源文件边界、57 个测试文件共 165 项测试、890 个模块构建。
- Playwright E2E 从 17 项增加至 25 项，全部通过。
- 本轮只修改 `frontend/**`，不改变后端接口、contracts 或 Phase 11 契约状态。
- 完整记录见 `FRONTEND_FIVE_ROLE_NAVIGATION_AUDIT.md`。
