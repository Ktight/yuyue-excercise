...
export const authHandlers = [
  http.post('/api/auth/login/', async ({ request }) => {
    ...
    return HttpResponse.json(errorBody('AUTH_INVALID_CREDENTIALS', '手机号或密码错误'), {
      status: 401,
    });
  }),
  ...
  http.post('/api/auth/refresh/', async ({ request }) => {
    ...
    if (body.refresh_token !== MOCK_REFRESH_TOKEN) {
      return HttpResponse.json(errorBody('AUTH_TOKEN_INVALID', '刷新令牌无效'), { status: 401 });
    }
  ...
  http.get('/api/auth/me/', ({ request }) => {
    if (!request.headers.get('Authorization')) {
      return HttpResponse.json(errorBody('AUTH_UNAUTHORIZED', '未登录'), { status: 401 });
    }
  ...
  http.post('/api/auth/change-password/', async ({ request }) => {
    ...
    if (body.old_password !== '12345678') {
      return HttpResponse.json(errorBody('AUTH_WRONG_PASSWORD', '原密码不正确'), { status: 400 });
    }
